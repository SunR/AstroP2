...
trainingSetLabels = trainingSet[:,3]  #putting labels in separate array
...
for i in trainingSetLabels:
    if i == -1:
        counter +=1
print counter
trainingSet = trainingSet[:, 1:2] #removing label cols from actual inputs
startTime = time.time()
...
testingSetLabels = testingSet[:,3]  #putting labels in separate array
testingSetLabels[testingSetLabels == 0] = -1 #replacing all 0 with -1 to match sklearn format
testingSet = testingSet[:, 1:2] #removing label cols from actual inputs

testingAccuracy = clf.score(testingSet, testingSetLabels)
print "Testing accuracy = ", testingAccuracy
print "Time = ", time.time() - startTime, "seconds"

classificationSet = uncertains[:, 1:2]
...
uncertainsPredictions = np.column_stack((uncertains[:, 0:2], predictions)) #hstack doesn't work here, b/c multidimensional array?
